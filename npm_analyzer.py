...
logging.basicConfig(
    ...
    handlers=[
        RotatingFileHandler(
            "npm_analyzer.log", maxBytes=5 * 1024 * 1024, backupCount=2
        ),
...
class MarkdownRenderer:
    ...
    def _setup_tags(self):
        ...
        self.text_widget.tag_config(
            "h1", foreground=Theme.ACCENT, font=("Segoe UI", 18, "bold"), spacing3=10
        )
        self.text_widget.tag_config(
            "h2", foreground=Theme.ACCENT, font=("Segoe UI", 16, "bold"), spacing3=8
        )
        self.text_widget.tag_config(
            "h3", foreground=Theme.ACCENT, font=("Segoe UI", 14, "bold"), spacing3=6
        )
        self.text_widget.tag_config(
            "h4", foreground=Theme.ACCENT, font=("Segoe UI", 12, "bold"), spacing3=4
        )
        self.text_widget.tag_config(
            "h5", foreground=Theme.ACCENT, font=("Segoe UI", 11, "bold")
        )
        self.text_widget.tag_config(
            "h6", foreground=Theme.ACCENT, font=("Segoe UI", 10, "bold")
        )
        ...
        self.text_widget.tag_config(
            "pre", background=Theme.BG_TERTIARY, font=("Consolas", 10)
        )
        ...
        self.text_widget.tag_config(
            "table_header", font=("Segoe UI", 10, "bold"), background=Theme.BG_TERTIARY
        )
    ...
    def render(self, markdown_text: str):
        ...
        try:
            ...
            logger.error(f"Error rendering markdown: {str(exception)}")
    ...
    def _format_inline_text(self, text: str) -> str:
        ...
        # Links [text](url)
        text = re.sub(
            r"\[([^\]]+)\]\(([^)]+)\)", lambda m: f"[{m.group(1)}]({m.group(2)})", text
        )
        return text
            ...
            if unit:
                ...
                if unit == "KB":
                    ...
                    return self._interpolate_color(
                        Theme.SIZE_KB, Theme.ACCENT_DARK, ratio
                    )
                elif unit == "MB":
                    ...
                    return self._interpolate_color(
                        Theme.SIZE_GB, Theme.ERROR_DARK, ratio
                    )
...
class SettingsManager:
    ...
    def _load_settings(self):
        ...
        try:
            ...
            logger.error(f"Error loading settings: {str(exception)}")
            self.config.read_dict(self.DEFAULT_SETTINGS)
            self._save_settings()
    ...
    def _save_settings(self):
        ...
        try:
            ...
            logger.error(f"Error saving settings: {str(exception)}")
...
class CacheManager:
    ...
    def _init_db(self):
        ...
        try:
            ...
            self.conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_last_fetched ON packages(last_fetched)"
            )
            self.conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_dependency_name ON package_dependencies(dependency_name)"
            )
            self.conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_dependent_name ON package_dependents(dependent_name)"
            )

            self.conn.commit()
        except Exception as e:
            logger.error(f"Database initialization error: {str(exception)}")
            self.conn = None
    ...
    def get_package(self, name: str, version: str = "latest") -> Optional[PackageInfo]:
        ...
        try:
            ...
            logger.error(f"Error getting package from cache: {str(exception)}")
            return None
    ...
    def _get_dependency_details(self, package_key: str) -> Dict[str, Dict]:
        ...
        try:
            ...
            logger.error(f"Error getting dependency details: {str(exception)}")
            return {}
    ...
    def _get_dependent_details(self, package_key: str) -> Dict[str, Dict]:
        ...
        try:
            ...
            logger.error(f"Error getting dependent details: {str(exception)}")
            return {}
    ...
    def save_package(self, package: PackageInfo):
        ...
        try:
            ...
            self._save_dependency_details(
                data["cache_key"], data.get("dependency_details", {})
            )

            # Save dependent details
            self._save_dependent_details(
                data["cache_key"], data.get("dependent_details", {})
            )

            self.conn.commit()
        except Exception as e:
            logger.error(f"Cache save error for {package.name}: {str(exception)}")
            self.conn.rollback()
            ...
            for dep_name, dep_data in details.items():
                ...
                """,
                    (
                        package_key,
                        dep_name,
                        dep_data.get("version"),
                        dep_data.get("size"),
                        dep_data.get("files"),
                        dep_data.get("last_publish"),
                    ),
                )
        except Exception as e:
            logger.error(f"Error saving dependency details: {str(exception)}")
            self.conn.rollback()
    ...
    def _save_dependent_details(self, package_key: str, details: Dict[str, Dict]):
        ...
        try:
            ...
            logger.error(f"Error saving dependent details: {str(exception)}")
            self.conn.rollback()
    ...
    def get_stats(self) -> Dict:
        ...
        try:
            ...
            logger.error(f"Error getting cache stats: {str(exception)}")
            return {"total": 0, "fresh": 0, "expired": 0, "size": 0}
    ...
    def clear_expired(self):
        ...
        try:
            ...
            logger.error(f"Error clearing expired cache: {str(exception)}")
            self.conn.rollback()
    ...
    def clear_all(self):
        ...
        try:
            ...
            logger.error(f"Error clearing cache: {str(exception)}")
            self.conn.rollback()
...
class SearchHistoryManager:
    ...
    def _init_db(self):
        ...
        try:
            ...
            self.conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_mode ON search_history(mode)"
            )

            self.conn.commit()
        except Exception as e:
            logger.error(
                f"Search history database initialization error: {str(exception)}"
            )
            self.conn = None
    ...
    def add_search(
        ...
        try:
            ...
            logger.error(f"Error adding search to history: {str(exception)}")

    def get_recent_searches(
        self, limit: int = MAX_SEARCH_HISTORY
    ) -> List[Tuple[str, str]]:
        ...
        try:
            ...
            logger.error(f"Error getting search history: {str(exception)}")
            return []
    ...
    def get_search_stats(self) -> Dict:
        ...
        try:
            ...
            logger.error(f"Error getting search stats: {str(exception)}")
            return {}
    ...
    def clear_history(self):
        ...
        try:
            ...
            logger.error(f"Error clearing search history: {str(exception)}")
    ...
    def get_popular_queries(self, limit: int = 5) -> List[Tuple[str, int]]:
        ...
        try:
            ...
            logger.error(f"Error getting popular queries: {str(exception)}")
            return []
...
class NPMClient:
    ...
    def __init__(self, cache: CacheManager, settings: SettingsManager):
        ...
        self.concurrency = settings.get_int(
            "General", "max_concurrent_requests", DEFAULT_MAX_CONCURRENT_REQUESTS
        )
        self.max_retries = 3
    ...
    def _get_file_info_from_npm_view(
        ...
        try:
            ...
            logger.error(f"Error getting file info from npm view: {str(exception)}")

        return None, None

    def _scrape_npm_web_page(
        ...
        try:
            ...
            for script in script_tags:
                if "window.__context__" in str(script):
                    try:
                        ...
                        logger.error(
                            f"Error parsing JSON from script: {str(exception)}"
                        )
                        continue
            ...
            logger.error(
                f"Error scraping npm web page for {package_name}: {str(exception)}"
            )
            return None, None, None
    ...
    def _get_dependents_count(self, package_name: str) -> int:
        ...
        try:
            ...
            dependents_heading = soup.find(
                "h2", string=re.compile(r"Dependents \(?(\d+)\)?")
            )
            ...
            logger.error(
                f"Error getting dependents count for {package_name}: {str(exception)}"
            )

        return 0

    def _get_dependency_details(
        self, package_name: str, dependencies: List[str]
    ) -> Dict[str, Dict]:
        ...
        def fetch_dep_details(dep_name: str) -> Tuple[str, Dict[str, str]]:
            ...
            try:
                ...
                if response and response.status_code == 200:
                    ...
                    if latest_version and latest_version in data.get("versions", {}):
                        ...
                        result = {
                            ...
                            if latest_version in data.get("time", {})
                            else "Unknown",
                        }
                ...
                logger.error(f"Error fetching details for {dep_name}: {str(exception)}")
                return dep_name, {
                    "size": "Unknown",
                    "files": "Unknown",
                    "last_publish": "Unknown",
        ...
        with concurrent.futures.ThreadPoolExecutor(
            max_workers=min(10, limit)
        ) as executor:
            futures = [
                executor.submit(fetch_dep_details, dep_name)
                for dep_name in dependencies[:limit]
            ]
    ...
    def _extract_file_tree(self, package_name: str, version: str = "latest") -> Dict:
        ...
        try:
                ...
                with zipfile.ZipFile(package_path, "r") as zip_ref:
                    for file_info in zip_ref.infolist():
                        if not file_info.is_dir():
                            ...
                            self._add_to_file_tree(
                                file_tree, path_parts, file_info.file_size
                            )
            ...
            logger.error(
                f"Error extracting file tree for {package_name}: {str(exception)}"
            )
            return {}
    ...
    def get_comprehensive_data(self, package_name: str) -> Optional[PackageInfo]:
        ...
        try:
            ...
            if not latest_version or latest_version not in registry_data.get(
                "versions", {}
            ):
                return None
            ...
            package_info = PackageInfo(
                ...
                repository=repo_url,
                downloads_last_week=self._fetch_download_stats(package_name).get(
                    "downloads", 0
                ),
                ...
                peer_dependencies_count=len(peer_deps)
                ...
                or any("@types" in key for key in dependencies),
                has_tests=any(
                    key in dev_deps
                    for key in ["jest", "mocha", "jasmine", "ava", "tape"]
                ),
                has_readme=bool(readme_content),
                maintainers_count=len(registry_data.get("maintainers", [])),
                maintainers=[
                    m.get("name", "")
                    for m in registry_data.get("maintainers", [])
            ...
            logger.error(
                f"Error fetching comprehensive data for {package_name}: {str(exception)}"
            )
            return None
    ...
    def _fetch_readme(self, package_name: str, registry_data: Dict) -> str:
        ...
        repo_url = self._extract_repo_url(
            registry_data.get("versions", {})
            .get(registry_data.get("dist-tags", {}).get("latest", ""), {})
            .get("repository", {})
        )
    ...
    def _fetch_github_readme(self, repo_url: str) -> str:
        ...
        try:
            ...
            logger.error(f"Error fetching GitHub README: {str(exception)}")
    ...
    def _fetch_npmjs_readme(self, package_name: str) -> str:
        ...
        try:
            ...
            if readme_div:
                ...
                for element in readme_div.find_all(
                    [
                        "p",
                        "pre",
                        "h1",
            ...
            logger.error(f"Error fetching npmjs README: {str(exception)}")
    ...
    def _fetch_download_stats(self, package_name: str) -> Dict:
        ...
        try:
            ...
            if response:
                data = response.json()
                return {"downloads": data.get("downloads", 0), "trend": "stable"}
        except Exception as e:
            logger.error(
                f"Error fetching downloads for {package_name}: {str(exception)}"
            )
    ...
    def search_packages(
        ...
        int(size_max * 1024 * 1024) if size_max else None
        ...
        def fetch_page():
            ...
            while len(all_packages) < max_results:
                params = {"text": query, "size": page_size, "from": from_value}
                ...
                try:
                    ...
                    with concurrent.futures.ThreadPoolExecutor(
                        max_workers=min(10, self.concurrency)
                    ) as executor:
                        futures = []
                        ...
                        for result in results:
                            ...
                            if fetch_details:
                                futures.append(
                                    executor.submit(
                                        self.get_comprehensive_data, package_name
                                    )
                                )
                            else:
                                ...
                                try:
                                    ...
                                    if stats_response:
                                        downloads = stats_response.json().get(
                                            "downloads", 0
                                        )
                                except Exception:
                                    pass
                                ...
                                dependents_count = 0
                                try:
                                    dependents_count = self._get_dependents_count(
                                        package_name
                                    )
                                except Exception:
                                    pass
                                ...
                                try:
                                    ...
                                    if registry_response:
                                        ...
                                        latest_version = registry_data.get(
                                            "dist-tags", {}
                                        ).get("latest", "")
                                        if (
                                            latest_version
                                            and latest_version
                                            in registry_data.get("versions", {})
                                        ):
                                            version_info = registry_data["versions"][
                                                latest_version
                                            ]
                                            dependencies = version_info.get(
                                                "dependencies", {}
                                            )
                                            dependencies_count = (
                                                len(dependencies)
                                ...
                                try:
                                    ...
                                    if registry_response:
                                        ...
                                        if (
                                            latest_version
                                            and latest_version
                                            in registry_data.get("time", {})
                                        ):
                                            date_str = registry_data["time"][
                                                latest_version
                                            ]
                                            last_publish = self._format_publish_date(
                                ...
                                file_count = "Unknown"
                                try:
                                    size, files = self._get_file_info_from_npm_view(
                        ...
                        if fetch_details:
                            for future in concurrent.futures.as_completed(futures):
                                ...
                                if not skip_package and date_filter:
                                    try:
                                        if (
                                            pkg.modified_date != "N/A"
                                            and pkg.modified_date != "Unknown"
                                        ):
                                            pkg_date = dateutil.parser.parse(
                                                pkg.modified_date
                                            )
                    ...
                    logger.error(f"Error fetching page: {str(exception)}")
                    break
    ...
    def download_package(
        ...
        try:
            ...
            success = False
            error_message = str(e)
            logger.error(f"Download error for {package_name}: {str(exception)}")
        finally:
    ...
    def download_packages_concurrent(
        self,
        package_list: Sequence[Union[str, Dict]],
        progress_callback: Optional[Callable] = None,
    ) -> List[Dict]:
        ...
        results = []
        max_workers = min(
            5, self.settings.get_int("General", "max_concurrent_downloads", 5)
        )
        download_semaphore = threading.Semaphore(max_workers)
        result_lock = threading.Lock()

        def download_single_package(
            package_info: Union[str, Dict], index: int, total: int
        ):
            nonlocal results
            ...
            with download_semaphore:
                package_name = (
                    ...
                    else package_info.get("name", "")
                )
                version = (
                    package_info.get("version", "latest")
                ...
                try:
                    ...
                    logger.error(f"Error downloading {package_name}: {str(exception)}")
...
class FileTreeViewer:
    ...
    def _create_ui(self):
        ...
        ttk.Label(header, text="File Tree", font=("Segoe UI", 12, "bold")).pack(
            side=tk.LEFT
        )

        # Refresh button
        self.refresh_btn = ttk.Button(
            header, text="Refresh", style="Secondary.TButton", command=self.refresh_tree
        )
        ...
        content_scrollbar_y = ttk.Scrollbar(
            content_frame, orient=tk.VERTICAL, command=self.content_text.yview
        )
        content_scrollbar_x = ttk.Scrollbar(
            content_frame, orient=tk.HORIZONTAL, command=self.content_text.xview
        )

        self.content_text.configure(
            yscrollcommand=content_scrollbar_y.set,
            xscrollcommand=content_scrollbar_x.set,
        )
        ...
        for name, data in tree_data.items():
            if data["type"] == "directory":
                ...
                node = self.tree.insert(
                    parent, "end", text=name, values=("",), open=False
                )
                ...
                # Add file node
                self.tree.insert(
                    parent, "end", text=name, values=(data["size_str"],), tags=(name,)
                )
        ...
        try:
            ...
            if not download_result["success"]:
                self.content_text.insert(
                    tk.END, f"Error downloading package: {download_result['error']}"
                )
                return
            ...
            if not os.path.exists(package_path):
                self.content_text.insert(
                    tk.END, f"Package file not found: {package_path}"
                )
                return
                ...
                with zipfile.ZipFile(package_path, "r") as zip_ref:
                    for file_info in zip_ref.infolist():
                        if not file_info.is_dir() and file_info.filename == file_path:
                            content = zip_ref.read(file_info.filename).decode(
...
class NPMAnalyzerApp:
    ...
    def __init__(self, root: tk.Tk):
        ...
        self.cache = CacheManager(
            CACHE_DB, self.settings.get_int("General", "cache_ttl_days", CACHE_TTL_DAYS)
        )
        ...
        self.font = tkfont.Font(
            family="Segoe UI", size=self.settings.get_int("General", "font_size", 10)
        )
    ...
    def _create_ui(self):
        ...
        self.search_history_combo.bind(
        ...
        search_entry = ttk.Entry(
            search_container, textvariable=self.search_var, width=50
        )
        ...
        self.min_size_entry = ttk.Entry(
            size_frame, textvariable=self.min_size_var, width=8
        )
        ...
        worker_entry = ttk.Entry(worker_frame, textvariable=self.worker_var, width=5)
        ...
        self.date_label = ttk.Label(
            self.date_btn_frame, text="", foreground=Theme.ACCENT
        )
        ...
        ttk.Label(
            results_header, text="Search Results", font=("Segoe UI", 12, "bold")
        ).pack(side=tk.LEFT)
        self.results_count = ttk.Label(
            results_header, text="0 packages", foreground=Theme.TEXT_SECONDARY
        )
        ...
        ttk.Checkbutton(
            sel_frame, text="Auto-enrich", variable=self.fetch_details
        ).pack(side=tk.LEFT, padx=5)
        ...
        scrollbar_y = ttk.Scrollbar(
            tree_container, orient=tk.VERTICAL, command=self.results_tree.yview
        )
        scrollbar_x = ttk.Scrollbar(
            tree_container, orient=tk.HORIZONTAL, command=self.results_tree.xview
        )
        ...
        ttk.Label(status_container, textvariable=self.status_var, anchor=tk.W).pack(
    ...
    def _create_overview_tab(self):
        ...
        overview_scrollbar = ttk.Scrollbar(
            overview_scroll, orient=tk.VERTICAL, command=overview_canvas.yview
        )
    ...
    def _create_dependencies_tab(self):
        ...
        deps_label = ttk.Label(
            deps_frame, text="Dependencies", font=("Segoe UI", 10, "bold")
        )
        ...
        deps_scrollbar_y = ttk.Scrollbar(
            deps_scroll, orient=tk.VERTICAL, command=self.deps_tree.yview
        )
        ...
        dependents_label = ttk.Label(
            dependents_frame, text="Dependents", font=("Segoe UI", 10, "bold")
        )
        ...
        dependents_scrollbar_y = ttk.Scrollbar(
            dependents_scroll, orient=tk.VERTICAL, command=self.dependents_tree.yview
        )
    ...
    def _create_json_tab(self):
        ...
        json_scrollbar = ttk.Scrollbar(
            json_scroll, orient=tk.VERTICAL, command=json_canvas.yview
        )
...
TTL: {self.cache.ttl_days} days"""
    ...
    def _clear_cache(self):
        if messagebox.askyesno("Clear Cache", "Clear expired cache entries?"):
            ...
            stats = self.cache.get_stats()
            messagebox.showinfo(
                "Cache Cleared", f"Cache cleared. {stats['total']} entries remaining."
            )
    ...
    def _update_worker_count(self, event=None):
        ...
        try:
            ...
            self.worker_display.config(
                text=f"{workers} Workers | {npm_status}", foreground=npm_color
            )
    ...
    def _search_packages(self, event=None):
        ...
        if date_option == "last week":
            ...
            date_filter = datetime.datetime.combine(
                self.selected_date, datetime.time.min
            )
        ...
        def perform_search():
            try:
                ...
                def progress_callback(current: int, total: int, max_results: int):
                    ...
                    self.root.after(
                        ...
                        lambda: self.status_var.set(
                            f"Fetching: {current}/{max_results}"
                        ),
                    )
                ...
                def result_callback(packages: List[PackageInfo]):
                    for pkg in packages:
                        self.root.after(
                            0, lambda p=pkg: self._add_package_to_results(p)
                        )
                ...
                self.root.after(
                    0,
                    lambda: self.status_var.set(
                        f"Found {len(packages)} packages in {elapsed:.1f}s"
                    ),
                )
                self.root.after(
                    0,
                    lambda: self.results_count.config(text=f"{len(packages)} packages"),
                )

                stats = self.client.get_stats()
                self.root.after(
                    0,
                    lambda: self.stats_label.config(
                        text=f"Requests: {stats['requests']} | Cache: {stats['cache_hit_rate']}"
                    ),
                )
            except Exception as e:
                logger.error(f"Search error: {str(exception)}")
            ...
            if package_name:
                ...
                def fetch():
                    try:
                        ...
                        logger.error(f"Error loading package: {str(exception)}")
                        self.root.after(
    ...
    def _display_package(self, pkg: PackageInfo):
        ...
        ttk.Label(header, text=pkg.name, font=("Segoe UI", 14, "bold")).pack(
            anchor=tk.W, padx=15, pady=(10, 2)
        )
        ttk.Label(header, text=f"v{pkg.version}", foreground=Theme.TEXT_SECONDARY).pack(
            anchor=tk.W, padx=15, pady=(0, 5)
        )
        ...
        if pkg.description:
            ...
            ttk.Label(
                desc_frame, text=pkg.description, wraplength=600, justify=tk.LEFT
            ).pack(anchor=tk.W)
        ...
        if pkg.readme and pkg.readme.strip():
            ...
            ttk.Label(readme_frame, text="README", font=("Segoe UI", 12, "bold")).pack(
            ...
            # Add scrollbar
            readme_scrollbar = ttk.Scrollbar(
                readme_container, orient=tk.VERTICAL, command=self.readme_text.yview
            )
        ...
        info_data = [
            ...
            (
                "Weekly Downloads",
                str(pkg.downloads_last_week) if pkg.downloads_last_week else "N/A",
            ),
            ...
            (
                "Dependencies",
                f"{pkg.dependencies_count} ({len(pkg.dependencies) if pkg.dependencies else 0} direct)",
            ),
        ...
        for label, value in info_data:
            if value and value != "Unknown" and value != "N/A" and value != "0":
                ...
                ttk.Label(
                    row, text=value, font=("Segoe UI", 9, "bold"), anchor=tk.W
                ).pack(side=tk.LEFT, fill=tk.X, expand=True)

        # URLs section
        if pkg.homepage or pkg.repository:
            ttk.Separator(self.overview_content, orient=tk.HORIZONTAL).pack(
                fill=tk.X, pady=15
            )
            ...
            if pkg.homepage:
                ...
                ttk.Label(homepage_frame, text="Homepage:", width=10, anchor=tk.W).pack(
                    side=tk.LEFT
                )
            ...
            if pkg.repository:
                ...
                ttk.Label(repo_frame, text="Repository:", width=10, anchor=tk.W).pack(
                    side=tk.LEFT
                )
                repo_btn = ttk.Button(
                    repo_frame,
        ...
        if pkg.keywords:
            ...
            ttk.Label(
                self.overview_content, text="Keywords", font=("Segoe UI", 10, "bold")
            ).pack(anchor=tk.W, pady=(0, 10), padx=5)
        ...
        if pkg.dependency_details:
            ...
            self.deps_tree.insert(
                "", "end", text="", values=("No dependencies found", "", "", "", "")
            )

        if pkg.dependents:
            for dep_name in pkg.dependents[:20]:  # Limit to 20 dependents
                self.dependents_tree.insert(
                    "", "end", text="", values=(dep_name, "N/A", "N/A", "N/A")
                )
            ...
            self.dependents_tree.insert(
                ...
                "end",
                text="",
                values=(f"{pkg.dependents_count} dependents (not loaded)", "", "", ""),
            )
        else:
            self.dependents_tree.insert(
                "", "end", text="", values=("No dependents found", "", "", "")
            )
    ...
    def _confirm_and_download(self, packages: List[str], description: str):
        ...
        batch_dir = os.path.join(
            download_dir,
            f"npm_downloads_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}",
        )
        ...
        def do_download():
            try:
                ...
                results = self.client.download_packages_concurrent(
                    packages, progress_callback=progress_callback
                )
                ...
                if failed:
                    failed_msg = "\nFailed downloads:\n" + "\n".join(
                        [f"- {pkg['package']}: {pkg['error']}" for pkg in failed[:5]]
                    )
                ...
                self.root.after(
                    0, lambda: messagebox.showerror("Download Error", str(e))
                )
            finally:
                self.root.after(0, lambda: self.progress.configure(value=100))
                self.root.after(0, lambda: self.root.config(cursor=""))
                    ...
                    if pkg and pkg.repository:
                        ...
                        self.root.after(
                            ...
                            lambda: messagebox.showinfo(
                                "No Repository", "No repository URL found"
                            ),
                        )
                except Exception as e:
                    logger.error(f"Error opening repository: {str(exception)}")
                ...
                try:
                    ...
                    if pkg and pkg.homepage:
                        ...
                        self.root.after(
                            0,
                            lambda: messagebox.showinfo(
                                "No Homepage", "No homepage URL found"
                            ),
                        )
                except Exception as e:
                    logger.error(f"Error opening homepage: {str(exception)}")
    ...
    def open_url(self, url: str):
        ...
        try:
            ...
            logger.error(f"Error opening URL: {str(exception)}")
...
if __name__ == "__main__":
    root = tk.Tk()
    _app = NPMAnalyzerApp(root)  # noqa: F841
    root.mainloop()
...
class NpmAPI:
    ...
    def search_packages(
        ...
        def fetch_page(page_num):
            ...
            try:
                ...
                print(f"Error searching page {page_num}: {str(exception)}")
                return []
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            future_to_page = {
                executor.submit(fetch_page, i): i for i in range(pages_to_fetch)
            }
    ...
    def get_package_details(self, package_name):
        ...
        if latest_version != "Unknown" and latest_version in package_info.get(
            "versions", {}
        ):
            ...
            if "time" in package_info and latest_version in package_info["time"]:
                ...
                try:
                    ...
                    time_obj = datetime.datetime.fromisoformat(
        ...
        try:
            ...
            for selector in size_selectors:
                ...
                if size_element and "Unpacked Size" in size_element[0].get_text():
                    ...
                    size_match = re.search(
                        r"Unpacked Size:\s*([\d\.]+\s*[KMG]?B)", size_text
                    )
                    if size_match:
            ...
            if (
                details["last_published"] == "Unknown"
            ):  # Only if not already set from API data
                time_selectors = [
            ...
            for selector in dependents_selectors:
                try:
                    ...
                    if dependents_element:
                        ...
                        if dependents_match:
                            details["dependents_count"] = dependents_match.group(
                                1
                            ).strip()
                            break
                except Exception:
            ...
            if (
                "dependents_count" in details
                and details["dependents_count"] != "Unknown"
            ):
                try:
                    # We have a count, but we want some actual dependents for display
                    # Just grab a few from the first page as examples
                    dependents_url = (
                        f"https://www.npmjs.com/browse/depended/{package_name}"
                    )
                    ...
                    print(f"Error fetching dependents: {str(exception)}")

        except requests.RequestException as e:
            print(f"Error fetching package details from npm website: {str(exception)}")
        ...
        if details["file_count"] == "Unknown" and details["dependencies"]:
            ...
            if deps_count > 0:
                ...
                estimated_files = (
                    deps_count * 3
                )  # 3 files per dependency as a wild guess
    ...
    def get_package_info(self, package_name):
        ...
        try:
            ...
            print(f"Error getting package info for {package_name}: {str(exception)}")
            return None

    def get_dependencies(
        self, package_name, include_dev=False, max_depth=5, progress_callback=None
    ):
            ...
            if depth < max_depth:
                ...
                if include_dev:
                    dev_dependencies = list(
                        latest_info.get("devDependencies", {}).keys()
                    )
                ...
                if progress_callback:
                    progress_callback(
                        total_processed, total_processed + dependency_queue.qsize()
                    )
    ...
    def get_dependents(self, package_name, max_pages=10, progress_callback=None):
        ...
        def scrape_page(page_num):
            ...
            try:
                ...
                print(f"Error fetching dependents page {page_num}: {str(exception)}")
                return []
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            future_to_page = {
                executor.submit(scrape_page, i): i for i in range(1, max_pages + 1)
            }
                ...
                if not page_results and future_to_page[future] > 1:
                    ...
                    for pending_future, page_num in future_to_page.items():
                        if (
                            ...
                            and page_num > future_to_page[future]
                        ):
                            pending_future.cancel()
...
class NpmDownloaderUI:
    ...
    def setup_ui(self):
        ...
        ttk.Radiobutton(
            search_type_frame,
            text="Package Name Search",
            variable=self.search_type_var,
            value="package",
            command=self.toggle_search_type,
        ).pack(side=tk.LEFT, padx=5)
        ttk.Radiobutton(
            search_type_frame,
            text="General Search",
        ...
        self.package_frame = ttk.LabelFrame(
            main_frame, text="Package Name Search", padding=10
        )
        ttk.Label(self.package_frame, text="Enter Package Name:").pack(
            anchor=tk.W, pady=5
        )
        self.package_name_var = tk.StringVar()
        ttk.Entry(
            self.package_frame, textvariable=self.package_name_var, width=50
        ).pack(fill=tk.X, pady=5)
        ttk.Label(self.package_frame, text="Example: graphlit-client").pack(
            anchor=tk.W, pady=2
        )
        ttk.Button(self.package_frame, text="OK", command=self.search_package).pack(
            anchor=tk.E, pady=5
        )

        # General search frame
        self.general_frame = ttk.LabelFrame(
            main_frame, text="General Search", padding=10
        )

        # Search query input
        ttk.Label(self.general_frame, text="Search Query:").grid(
            row=0, column=0, sticky=tk.W, pady=5
        )
        self.search_query_var = tk.StringVar()
        ttk.Entry(
            self.general_frame, textvariable=self.search_query_var, width=50
        ).grid(row=0, column=1, sticky=tk.W + tk.E, padx=5, pady=5)

        # Search filters
        filters_frame = ttk.LabelFrame(
            self.general_frame, text="Search Filters", padding=5
        )
        filters_frame.grid(row=1, column=0, columnspan=2, sticky=tk.W + tk.E, pady=5)

        # Time filter
        ttk.Label(filters_frame, text="Time Filter:").grid(
        ...
        for text, value in time_options:
            ttk.Radiobutton(
                time_filter_frame, text=text, variable=self.time_filter_var, value=value
            ).pack(side=tk.LEFT, padx=5)

        # Results count filter
        ttk.Label(filters_frame, text="Max Results:").grid(
        ...
        results_options = [("100", 100), ("500", 500), ("1000", 1000), ("All", 10000)]

        for text, value in results_options:
            ttk.Radiobutton(
                max_results_frame, text=text, variable=self.max_results_var, value=value
            ).pack(side=tk.LEFT, padx=5)

        # Search button
        ttk.Button(self.general_frame, text="Search", command=self.search_general).grid(
            row=2, column=1, sticky=tk.E, pady=10
        )

        # Package details frame
        self.details_frame = ttk.LabelFrame(
            main_frame, text="Package Details", padding=10
        )

        # Treeview for package details
        self.details_tree = ttk.Treeview(
            self.details_frame, columns=("property", "value"), show="headings", height=6
        )
        ...
        ttk.Button(
            ...
            command=lambda: self.download_package_option("package"),
        ).pack(side=tk.LEFT, padx=5)
        ttk.Button(
            download_frame,
            text="Download Dependencies",
            command=lambda: self.download_package_option("dependencies"),
        ).pack(side=tk.LEFT, padx=5)
        ttk.Button(
            download_frame,
            text="Download Dependants",
            command=lambda: self.download_package_option("dependants"),
        ).pack(side=tk.LEFT, padx=5)

        # Results frame for general search
        self.results_frame = ttk.LabelFrame(
            main_frame, text="Search Results", padding=10
        )

        # Create treeview for search results
        self.results_tree = ttk.Treeview(
            self.results_frame,
            columns=("name", "version", "description", "size", "files", "date"),
            show="headings",
            height=10,
        )
    ...
    def toggle_search_type(self):
        ...
        if self.search_type_var.get() == "package":
            ...
            self.output_text.insert(
                tk.END, "Enter a package name and click OK to see package details.\n"
            )
            ...
            self.output_text.delete(1.0, tk.END)
            self.output_text.insert(
                tk.END, "Enter a search query and select filters, then click Search.\n"
            )
    ...
    def search_package(self):
        ...
        self.output_text.delete(1.0, tk.END)
        self.output_text.insert(
            tk.END, f"Fetching package details for: {package_name}\n"
        )
        ...
        def fetch_details():
            try:
                ...
                if package_details:
                    self.current_package = package_name
                    self.root.after(
                        0, lambda: self.display_package_details(package_details)
                    )
                else:
                    self.root.after(
                        0,
                    ...
                    self.root.after(
                        0,
                        lambda: self.output_text.insert(
                            tk.END, f"Package '{package_name}' not found\n"
                        ),
                    )
            except Exception:
                self.root.after(
                    0,
                    lambda: messagebox.showerror(
                        "Error", f"Error fetching package details: {str(e)}"
                    ),
                )
                self.root.after(
                    0, lambda: self.output_text.insert(tk.END, f"Error: {str(e)}\n")
                )
            finally:
                self.root.after(0, lambda: self.root.config(cursor=""))
                self.root.after(0, lambda: self.status_var.set("Ready"))
        ...
        def perform_search():
            try:
                ...
                def update_progress(current, total):
                    percent = (current / total) * 100
                    self.root.after(
                        0, lambda: self.progress_bar.configure(value=percent)
                    )
                    self.root.after(
                        0,
                        lambda: self.status_var.set(
                            f"Searching: {current}/{total} pages..."
                        ),
                    )
                ...
                if search_results:
                    ...
                    self.root.after(
                        0,
                        lambda: self.status_var.set(
                            f"Found {len(search_results)} results. Processing details..."
                        ),
                    )
                    self.root.after(
                        0,
                        lambda: self.output_text.insert(
                            tk.END,
                            f"Found {len(search_results)} packages. Processing details...\n",
                        ),
                    )
                    ...
                    for batch_index, batch in enumerate(batches):
                        self.root.after(
                            0,
                            lambda bi=batch_index, bt=len(batches): self.status_var.set(
                                f"Processing batch {bi + 1}/{bt} ({batch_size} packages each)..."
                            ),
                        )
                        self.root.after(
                            0,
                            lambda bi=batch_index,
                            bt=len(batches): self.progress_bar.configure(
                                value=(bi / bt) * 100
                            ),
                        )
                        ...
                        for result in batch:
                            try:
                                ...
                                item_id = self.root.after(
                                    0,
                                    lambda pkg=result_entry: self.results_tree.insert(
                                        "",
                                        "end",
                                        values=(
                                            pkg["name"],
                                    ...
                                    try:
                                        ...
                                        if details:
                                            ...
                                            results_with_details[result_idx]["size"] = (
                                                details.get("unpacked_size", "Unknown")
                                            )
                                            results_with_details[result_idx][
                                                "files"
                                            ] = details.get("file_count", "Unknown")

                                            # Update the tree item
                                            self.root.after(
                                                0,
                                                lambda: self.results_tree.item(
                                                    tree_item,
                                                    values=(
                                                        pkg_name,
                                                        results_with_details[
                                        ...
                                        print(
                                            f"Error updating details for {pkg_name}: {str(e)}"
                                        )
                    ...
                    self.output_text.insert(
                        tk.END,
                        f"Processed {len(results_with_details)} packages with details.\n",
                    )
                    self.output_text.insert(
                        tk.END, "Double-click on a package to see more details.\n"
                    )
                    self.status_var.set(
                        f"Ready - Found {len(results_with_details)} packages"
                    )
                    ...
                    self.root.after(
                        0,
                        lambda: self.output_text.insert(
                            tk.END, "No packages found matching your query.\n"
                        ),
                    )
                    self.root.after(
                        0, lambda: self.status_var.set("Ready - No results found")
                    )
    ...
    def display_package_details(self, details):
        ...
        self.details_tree.insert(
            "", "end", values=("Description", details["description"])
        )
        self.details_tree.insert(
            "", "end", values=("Unpacked Size", details["unpacked_size"])
        )
        self.details_tree.insert(
            "", "end", values=("Total Files", details["file_count"])
        )

        if "dependents_count" in details:
            self.details_tree.insert(
                "", "end", values=("Dependents Count", details["dependents_count"])
            )

        # Display dependencies count
        dep_count = len(details["dependencies"])
        self.details_tree.insert(
            "", "end", values=("Dependencies Count", str(dep_count))
        )
        ...
        self.output_text.insert(
            tk.END, f"Package: {details['name']} v{details['version']}\n"
        )
        ...
        if "dependents_count" in details:
            self.output_text.insert(
                tk.END, f"Dependents: {details['dependents_count']}\n"
            )
    ...
    def download_package_option(self, option_type):
        ...
        download_dir = filedialog.askdirectory(
            initialdir=os.getcwd(), title="Select Download Directory"
        )
        ...
        try:
            ...
            messagebox.showerror(
                "Error", f"Could not create directory for package: {str(exception)}"
            )
            return
        ...
        if option_type == "package":
            ...
            def fetch_and_download_deps():
                try:
                    # Fetch dependencies
                    deps = self.api.get_dependencies(
                        self.current_package, include_dev=False
                    )
                    if deps:
                        ...
                        self.root.after(
                            0, lambda: self._confirm_and_download(packages_to_download)
                        )
                    else:
                        self.root.after(
                            0,
                            lambda: self.output_text.insert(
                                tk.END,
                                f"No dependencies found for {self.current_package}\n",
                            ),
                        )
                        self.root.after(0, lambda: self.status_var.set("Ready"))
                except Exception:
                    self.root.after(
                        0,
                        lambda: messagebox.showerror(
                            "Error", f"Error fetching dependencies: {str(e)}"
                        ),
                    )
                    self.root.after(
                        0, lambda: self.output_text.insert(tk.END, f"Error: {str(e)}\n")
                ...
                try:
                    ...
                    if deps:
                        ...
                        self.root.after(
                            ...
                            lambda: self.output_text.insert(
                                tk.END,
                                f"No dependants found for {self.current_package}\n",
                            ),
                        )
                        self.root.after(0, lambda: self.status_var.set("Ready"))
                except Exception:
                    self.root.after(
                        0,
                        lambda: messagebox.showerror(
                            "Error", f"Error fetching dependants: {str(e)}"
                        ),
                    )
                    self.root.after(
                        0, lambda: self.output_text.insert(tk.END, f"Error: {str(e)}\n")
    ...
    def _confirm_and_download(self, packages):
        ...
        self.output_text.insert(
            tk.END, f"Starting download of {len(packages)} package(s)...\n"
        )
        ...
        def do_download():
            try:
                ...
                results = self.api.download_packages_concurrent(
                    packages, progress_callback=self._download_progress_callback
                )
                ...
                self.root.after(
                    0,
                    lambda: self.status_var.set(
                        f"Ready - Downloaded {success_count}/{len(packages)} packages"
                    ),
                )
            except Exception:
                self.root.after(
    ...
    def _download_progress_callback(self, current, total, result):
        ...
        if success:
            self.root.after(
                0,
                lambda: self.output_text.insert(
                    tk.END, f"Downloaded {package} -> {os.path.basename(filename)}\n"
                ),
            )
        else:
            self.root.after(
                0,
                lambda: self.output_text.insert(
                    tk.END, f"Failed to download {package}: {error}\n"
                ),
            )
