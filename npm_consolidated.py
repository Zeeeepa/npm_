...
class NPMAnalyzerApp:
    ...
    def _setup_ui(self):
        ...
        ttk.Label(search_frame, text="Search NPM Packages:").pack(
        ...
        self.search_btn = ttk.Button(
            search_frame, text="🔍 Search", command=self.search_packages
        )
        ...
        ttk.Spinbox(
            search_frame, from_=10, to=100, textvariable=self.limit_var, width=5
    ...
    def _setup_package_list(self, parent):
        ...
        self.package_listbox = tk.Listbox(
            list_frame, yscrollcommand=list_scroll.set, font=("Courier", 10)
        )
    ...
    def _setup_package_details(self, parent):
        ...
        self.overview_text = scrolledtext.ScrolledText(
            overview_frame, wrap=tk.WORD, font=("Courier", 10)
        )
        ...
        self.deps_text = scrolledtext.ScrolledText(
            deps_frame, wrap=tk.WORD, font=("Courier", 10)
        )
        ...
        self.readme_text = scrolledtext.ScrolledText(
            readme_frame, wrap=tk.WORD, font=("Courier", 10)
        )
    ...
    def _apply_theme(self):
        ...
        self.package_listbox.configure(
            bg=THEME["bg_secondary"], fg=THEME["text"], selectbackground=THEME["accent"]
        )
    ...
    def search_packages(self):
        ...
        def search_thread():
            try:
                ...
                self.root.after(0, lambda err=ex: self.show_error(f"Search failed: {err}"))
            finally:
...
"""
        ...
        if pkg.dependencies:
            self.deps_text.insert(
                tk.END, f"Dependencies ({len(pkg.dependencies)}):\n\n"
            )
    ...
    def display_file_tree(self, tree: Dict[str, Any]):
        ...
        def add_node(parent, node):
            ...
            item_id = self.file_tree.insert(
                parent, tk.END, text=f"{icon} {name}", values=(node.get("path", ""),)
            )
    ...
    def export_results(self):
        ...
        try:
            ...
            messagebox.showinfo(
                "Success", f"Exported {len(self.packages)} packages to {path}"
            )
